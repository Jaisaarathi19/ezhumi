...
export default function SignUp() {
  ...
  const verifySupabaseOtp = async (email: string, token: string) => {
    try {
  ...
  const validateForm = () => {
    if (!formData.name || !formData.email) {
      setError(t('signup.errors.nameEmailRequired'));
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(t('signup.errors.genericError'));
                ...
                <div className="bg-green-900/20 backdrop-blur-sm rounded-lg p-4 border border-green-500/30">
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      maxLength={6}
                      className="appearance-none relative block w-32 px-3 py-2 border border-gray-700 placeholder-gray-400 text-white bg-gray-800/50 backdrop-blur-sm rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 focus:z-10 sm:text-sm text-center tracking-widest"
                      placeholder="000000"
                    />
                    <button
                      type="button"
